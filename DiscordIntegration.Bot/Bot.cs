...
public class Bot
{
    ...
    public DiscordSocketClient Client => client ??= new DiscordSocketClient(new()
    {
    ...
    private async Task Init()
    {
        ...
        Server.Terminated += OnTerminated;
    ...
    private void OnTerminated(object? sender, TerminatedEventArgs ev)
    {
        _ = Server.Start();
    }
    private async Task OnMessageRecived(SocketMessage message)
    {
        ...
        if (!Program.Config.Channels[ServerNumber].CommandChannel.Any(x => x == message.Channel.Id)) return;

        if (string.IsNullOrEmpty(message.Content)) return;

        if (!message.Content.Contains("+")) return;

        string command = string.Join(' ', message.Content.Trim().Split('+').Skip(1));

        ErrorCodes canRunCommand = SlashCommandHandler.CanRunCommand((IGuildUser)message.Author, ServerNumber, command);
        if (canRunCommand != ErrorCodes.None)
        {
            await message.Channel.SendMessageAsync(embed: await ErrorHandlingService.GetErrorEmbed(canRunCommand));
            return;
        }

        try
        {
            Log.Debug(ServerNumber, nameof(OnMessageRecived), $"Sending {command}");
            await Server.SendAsync(new RemoteCommand(ActionType.ExecuteCommand, message.Channel.Id, command, message.Author.Id, message.Author.Username));
            await message.Channel.SendMessageAsync("Command sent.");
        }
        catch (Exception e)
        {
            Log.Error(ServerNumber, nameof(OnMessageRecived), e);
            await message.Channel.SendMessageAsync(embed: await ErrorHandlingService.GetErrorEmbed(ErrorCodes.Unspecified, e.Message));
        }
    }
                ...
                case ActionType.Log:
                    ...
                    if (Enum.TryParse(command.Parameters[0].ToString(), true, out ChannelType type))
                    {
                        foreach (LogChannel channel in Program.Config.Channels[ServerNumber].Logs[type])
                        {
                            ...
                            Messages[channel] += $"[{TimestampTag.FromDateTime(DateTime.Now, TimestampTagStyles.ShortTime)} - {DateTime.Now.ToString("dd.MM.yyyy")}] {command.Parameters[1]}\n";
                        }
                    ...
                    if (message.Value.Length > 1900)
                    {
                        ...
                        switch (message.Key.LogType)
                        {
                            case LogType.Embed:
                                ...
                                break;
                            case LogType.Text:
                                _ = Guild.GetTextChannel(message.Key.Id).SendMessageAsync(msg);
                                break;
                            default:
                    ...
                    else
                    {
                        ...
                        switch (message.Key.LogType)
                        {
                            case LogType.Embed:
                                ...
                                break;
                            case LogType.Text:
                                await Guild.GetTextChannel(message.Key.Id).SendMessageAsync(message.Value);
                                break;
                            default:
                                throw new ArgumentOutOfRangeException();
                        }
