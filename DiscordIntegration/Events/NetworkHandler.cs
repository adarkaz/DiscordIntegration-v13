...
#pragma warning disable SA1313 // Parameter names should begin with lower-case letter
    ...
    internal sealed class NetworkHandler
    {
        ...
        {
            if (!IPAddress.TryParse(Instance.Config?.IPAddress, out IPAddress ipAddress))
            {
                Log.Error($"[NET] {string.Format(Language.InvalidIPAddress, Instance.Config?.IPAddress)}");
                return;
            }

            ev.IPAddress = ipAddress;
            ev.Port = Instance.Config.Port;
